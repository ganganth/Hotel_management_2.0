...
const RoomView = (props) => {
    ...
    const handleRoomBooking = (room) => {
        ...
        const isFound = items.find(i =>
            i.reservationType === 'rooms'
            && i.id === props.singleRoom.id
            && (i.quantity + 1) >= props.singleRoom.totalRooms
        );

        if (isFound) {
            toast.warning(`No Rooms are available`)
        }
        else {
            const r = {
                id: room.id,
            ...
            toast.success(`${room.name} added to the cart`);
                        ...
                        <Col md={4}>
                            <div className='roomView-details-price-container'>
                                ...
                                <div className='d-flex flex-column align-items-center'>
                                    ...
                                    <button className='d-block btn btn-primary roomView-details-price-book-btn' onClick={() => handleRoomBooking(props.singleRoom)}>Book Room Now <MdBookmarkAdd /></button>
                                </div>
                            </div>
                        </Col>
                    </Row>
